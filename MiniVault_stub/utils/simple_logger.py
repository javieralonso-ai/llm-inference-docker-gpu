...
def log_event(level, component, message):
    ...
    # Also send to telemetry if available
    try:
        telemetry_script = Path(__file__).parent.parent / "telemetry" / "telemetry_client.sh"
        if telemetry_script.exists():
            subprocess.run([str(telemetry_script), level, component, message], 
                         capture_output=True, timeout=1)
    except:
        pass  # Telemetry is optional, don't fail on errors
